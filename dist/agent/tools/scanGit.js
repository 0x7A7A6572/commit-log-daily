...
/** Git 只读子命令白名单 */
const ALLOWED_COMMANDS = ['log', 'branch', 'diff', 'show', 'status'];
/** 参数中禁止出现的危险模式 */
const BLOCKED_PATTERNS = ['rm', 'push', 'reset', 'clean', '--hard', ';', '&&', '|', '>', '<'];
/**
 * 安全执行本地 Git 命令
 * 使用 execFile + 数组传参，不经过 Shell，杜绝命令注入
 */
async function safeGitExecute(projectPath, args) {
    if (!path.isAbsolute(projectPath)) {
        throw new GitExecutionError(`路径必须是绝对路径，收到 "${projectPath}"`, projectPath, args);
    }
    const subCommand = args[0];
    if (!subCommand || !ALLOWED_COMMANDS.includes(subCommand)) {
        throw new GitExecutionError(`不允许执行 git ${subCommand ?? 'undefined'}，仅支持 ${ALLOWED_COMMANDS.join(', ')}`, projectPath, args);
    }
    for (const arg of args) {
        for (const pattern of BLOCKED_PATTERNS) {
            if (arg.toLowerCase().includes(pattern)) {
                throw new GitExecutionError(`参数包含危险模式 "${pattern}"，已拒绝执行`, projectPath, args);
            }
        }
    }
...
/** 扫描本地 Git 仓库的工具 */
export const scanGitTool = tool(async ({ projectPath, since, until, author }) => {
    const args = [
        'log',
        '--all',
        `--format=${LOG_FORMAT}`,
    ...
    name: 'scanGit',
    description: '扫描指定项目在时间范围内的 Git 提交记录。返回结构化的 commit 列表，包含 hash、作者、日期、提交信息和分支名。所有分支都会被扫描（--all）。',
    schema: z.object({
...
//# sourceMappingURL=scanGit.js.map