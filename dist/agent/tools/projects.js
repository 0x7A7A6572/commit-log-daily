...
/** 列出已配置项目的工具 */
export const listProjectsTool = tool(async () => {
    const config = readConfig();
    if (config.projects.length === 0) {
        return '当前没有已配置的项目。';
    }
    const lines = config.projects.map((p) => `- ${p.name}: ${p.path}`);
...
/** 添加或更新项目的工具 */
export const addProjectTool = tool(async ({ name, filePath }) => {
    const absPath = path.resolve(filePath);
    // 校验路径存在
    if (!fs.existsSync(absPath)) {
        throw new AgentToolError(`路径不存在: ${absPath}`, 'addProject');
    }
    // 校验是 Git 仓库
    const gitDir = path.join(absPath, '.git');
    if (!fs.existsSync(gitDir)) {
        throw new AgentToolError(`路径不是 Git 仓库: ${absPath}`, 'addProject');
    }
    const config = readConfig();
    ...
    name: 'addProject',
    description: '添加或更新一个项目配置。需要项目名称和绝对路径。路径必须是存在的 Git 仓库。',
    schema: z.object({
...
/** 删除项目的工具 */
export const removeProjectTool = tool(async ({ name }) => {
    const config = readConfig();
